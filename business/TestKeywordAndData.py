...
def TestKeywordAndData():
    try:
        if excelList.__len__() > 0:
            for excelItem in excelList:
                # 根据Excel文件中的sheet名称获取sheet对象
                caseSheet = excelItem.getSheetByName("测试用例")
                # 获取测试用例sheet中是否执行列对象
                isExecuteColumn = excelItem.getColumn(caseSheet, testCase_isExecute)
                # 记录执行成功的测试用例个数
                successfulCase = 0
                # 记录需要执行的用例个数
                requiredCase = 0
                ...
                for idx, i in enumerate(isExecuteColumn[1:]):  # idx默认从0开始， excel 表格行从1开始,列从1开始
                    # 用例sheet中第一行为标题，无须执行
                    caseName = excelItem.getCellOfValue(
                    ...
                    if i.value.lower() == "y":
                        ...
                        if useFrameWorkName == "数据":
                            ...
                            # 数据驱动所使用的数据名
                            dataSheetName = excelItem.getCellOfValue(
                                caseSheet, rowNo=idx + 2,
                                colsNo=testCase_dataSourceSheetName)
                            # 获取第idx+1行测试用例的步骤sheet对象
                            stepSheetObj = excelItem.getSheetByName(stepSheetName)
                            # 获取第idx+1行测试用例使用的数据sheet对象
                            dataSheetObj = excelItem.getSheetByName(dataSheetName)

                            # 通过数据驱动框架执行添加联系人
                            result = dataDriverFun(excelItem, dataSheetObj, stepSheetName)
                            if result:
                                logging.info("用例[%s]执行成功" % caseName)
                                successfulCase += 1
                                writeTestResult(excelItem, caseSheet, rowNo=idx + 2,
                                                colsNo="testCase",
                                                testResult="pass")
                            else:
                            ...
                            result_keyword = TestKeyword(excelItem, caseName, stepSheetName, idx)
                            if result_keyword:
                                logging.info("用例[%s]执行成功" % caseName)
                                successfulCase += 1
                                writeTestResult(excelItem, caseSheet, rowNo=idx + 2,
